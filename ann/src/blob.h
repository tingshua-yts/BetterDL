...
public:
    ...
    {
      h_ptr_ = new ftype[n_ * c_ * h_ * w_];//todo: 这里类型有些矛盾，h_ptr_为ftype类型
    }
...
// reset the current blob with the new size information
    ...
    cudnnTensorDescriptor_t tensor() {// todo :有什么用
      if (is_tensor_)
        return tensor_desc_;

      cudnnCreateTensorDescripter(&tensor_desc_);
      cudaSetTensor4dDescripter(tensor_desc_,
                                UDNN_TENSOR_NCHW, CUDNN_DATA_FLOAT,
                                n_, c_, h_, w_);
      is_tensor_ = true;
      return tensor_desc_;
    }
    ...
    ftype* to(DeviceType target) {
      ...
      if (target == host) {
        ...
        cudaMemcpy(d_ptr_, ptr(), sizeof(ftype)*len, cudaMemcpyHostToDevice);
        ptr = d_ptr_;
      }
      return ptr;
    }
...
private:
    ...
    // 4-d tensor dim, is used to define the format for batches of 2D images(From cudnn doc)
    int n_ = 1; // batch size
