...
def my_model(features, labels, mode, params):
  ...
  h_pool_flat = tf.reshape(h_pool, [-1, FLAGS.num_filters * len(list(params["filter_sizes"]))]) # shape: (batch, len(filter_size) * embedding_size)
...
def make_cluster_def(cluster_spec):
    ...
    for job_name, tasks in sorted(cluster_spec.items()):
      try:
        # if job_name != "ps":
        #       continue
      ...
      for i, task_address in enumerate(tasks):
        try:
...
def get_dlrover_config():
      executor = base_executor.BaseExecutor()
      executor.get_cluster_info_by_tf_config()
      return executor.get_config()
...
def get_naive_config():
  ...
  cluster_def = make_cluster_def(cluster_spec)
  logging.info("aaaaaaaa")
  logging.info(cluster_def)
  logging.info("aaaaaaaa")
  ##########
...
def start_tf_server():
  task_type, task_id = base_executor.get_tf_config_task_type_and_index()
  tf_config = os.environ["TF_CONFIG"]
  tf_config = json.loads(tf_config)
...
def main(unused_argv):
  ...
  with open(json_path) as f:
    configf = json.load(f)
  FLAGS.pad_word = configf["pad_word"]
  if configf["train_size"] < FLAGS.shuffle_buffer_size:
    FLAGS.shuffle_buffer_size = configf["train_size"]
  ...
  config = get_naive_config()
  #config = get_dlrover_config()
  logging.info(config)

  server = start_tf_server()
  task_type, _ = base_executor.get_tf_config_task_type_and_index()
  if task_type == "ps":
  ...
  train_spec = tf.estimator.TrainSpec(
    input_fn=lambda: input_fn(path_train, path_words, FLAGS.shuffle_buffer_size, configf["num_oov_buckets"]),
    max_steps=FLAGS.train_steps
  )
  input_fn_for_eval = lambda: input_fn(path_eval, path_words, 0, configf["num_oov_buckets"])
