...
def run_training_loop(rank, num_gpus, train_loader, test_loader):
    ...
    net = TrainerNet(num_gpus=num_gpus) # todo: 在ps worker创建模型
    ...
    for i, (data, target) in enumerate(train_loader):
        with dist_autograd.context() as cid:
            model_output = net(data) # todo：每个train worker都调用ps worker的forward? 那多个worker的意义是什么?
            target = target.to(model_output.device)
            loss = F.nll_loss(model_output, target)
            if i % 5 == 0:
                print(f"Rank {rank} training batch {i} loss {loss.item()}")
            dist_autograd.backward(cid, [loss]) # todo ？在哪里计算的
            ...
            assert remote_method(
                ...
                cid) != {}
            opt.step(cid) # todo? 仅更新ps worker上的参数?
